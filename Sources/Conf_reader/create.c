...
/*   Updated: 2020/05/10 15:51:01 by wstygg           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec3f				create_color(char *conf, int i)
{
	...
	k = 0;
	if ((len = ft_split_size(split)) != 4)
		ft_crash("Error at [%d] line:\n%s\nIn content of \"color\' have to be"
				" a 3 numbers for r, g and b respectively!\n", i + 1, conf);
	while (++k < len)
	{
...
t_vec3f				create_pos(char *conf, int i)
{
	...
	k = 0;
	if ((len = ft_split_size(split)) != 4)
		ft_crash("Error at [%d] line:\n%s\nIn content of \"position\" have to "
				"be a 3 numbers for x, y and z respectively!\n", i + 1, conf);
	while (++k < len)
	{
		if (!is_double(split[k]))
