...
static double		calc_shine(const t_vec3f normal, const t_vec3f vec_l, const double intensity)
{
	double			n_dot_l;

	n_dot_l = vec3f_dot(normal, vec_l);
	if (n_dot_l > 0)
		return (intensity * n_dot_l / (vec3f_length(normal) * vec3f_length(vec_l)));
	return (0);
}

static double		calc_specular(const double specular, const double intensity, const t_tvec tvec, const t_vec3f vec_l)
{
	t_vec3f			vec_r;
	double			r_dot_v;
	...
	if (specular)
	{
		...
		if (r_dot_v > 0)
			return (intensity * SDL_pow(r_dot_v / (vec3f_length(vec_r) * vec3f_length(tvec.view)), specular));
	}
	return (0);
}

static double		get_intensity(const t_tvec tvec, const t_light light, const double specular, const t_rt rt)
{
	t_vec3f			vec_l;
	double			z_max;
	double			intensity;
...
static double		compute_light(const t_tvec tvec, const double specular, const t_rt rt)
{
	double			intensity;
	t_list			*lights;
	t_light			*light;
