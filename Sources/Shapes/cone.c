...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/10 15:58:05 by wstygg            #+#    #+#             */
/*   Updated: 2020/05/10 16:00:02 by wstygg           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (vec3f_sub(vec3f_sub(point, cone.center),
			vec3f_scale(vec3f_scale(cone.normal,
			1.0 + SDL_pow(SDL_tan(cone.radius / 2.0), 2.0)),
			vec3f_dot(dir, cone.normal) * rt.closest_t +
			vec3f_dot(vec3f_sub(rt.camera, cone.center), cone.normal))));
}
										...
										const t_object cone)
{
	...
	k[1] = 2 * (vec3f_dot(dir, oc) - p * vec3f_dot(dir, cone.normal) *
			vec3f_dot(oc, cone.normal));
	k[2] = vec3f_dot(oc, oc) - p * SDL_pow(vec3f_dot(oc, cone.normal), 2.0);
	discriminant = k[1] * k[1] - 4 * k[0] * k[2];
	if (discriminant < 0)
		return (DBL_MAX);
	t[1] = (-k[1] + SDL_sqrt(discriminant)) / (2 * k[0]);
	t[2] = (-k[1] - SDL_sqrt(discriminant)) / (2 * k[0]);
